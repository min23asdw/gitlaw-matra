...
const PdfButton = React.memo(({ pageNumber, side, onJumpToPage }: { pageNumber: number, side: 'left' | 'right', onJumpToPage?: (p: number, s: 'left' | 'right') => void }) => (
    <button
        onClick={(e) => {
            e.stopPropagation();
        ...
        aria-label={`Open PDF page ${pageNumber}`}
    >
