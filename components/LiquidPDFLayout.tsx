...
export default function LiquidPDFLayout({ leftData, rightData, leftMeta, rightMeta }: Props) {
    ...
    const isContentNarrow = centerWidth > 0 && centerWidth < 640;
    ...
    return (
        <div className="flex h-[calc(100vh-140px)] w-full max-w-[1920px] mx-auto overflow-hidden bg-slate-200 rounded-xl shadow-2xl border border-slate-300 relative group/main">
            ...
            <div ref={centerRef} className="flex-1 flex min-w-0 bg-white overflow-hidden relative z-0 transition-all duration-300">
                <div className={`absolute left-4 bottom-6 z-30 transition-all duration-300 ${showLeftPdf ? '-translate-x-20 opacity-0' : 'translate-x-0 opacity-100'}`}>
                ...
                <div className={`absolute right-4 bottom-6 z-30 transition-all duration-300 ${showRightPdf ? 'translate-x-20 opacity-0' : 'translate-x-0 opacity-100'}`}>
